...
1. **Switching costs collapsed.** Buyers trial aggressively, run vendors head-to-head, and increasingly test whether a free or general-purpose LLM tool covers the job. The evaluation never really ends.
2. **Expansion got harder.** Budgets are scrutinized, seats are trimmed, and "we'll grow into the plan" conversations die in procurement. When expansion stalls, every churn point hits NRR directly.
3. **Nobody is watching the signals.** Usage data lives in product analytics. Ticket history lives in the support desk. Engagement lives in the CRM. Churn is visible for months in systems that don't talk to each other — and surfaces only as a renewal-call surprise.
